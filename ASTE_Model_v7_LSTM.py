"""
ASTE Pipeline v7 — DeBERTa-v3-base + LSTM
===============================================
Unified single-encoder model with LSTM + BIO tagger + VA regressor.
Improvements:
  - Added BiLSTM layer after Transformer encoder for better classification sequence modeling
  - Class-weighted BIO loss (handles tag imbalance)
  - Differential learning rates
  - Robust local folder paths
"""
...
DATA_DIR    = BASE_DIR / "nlp_assignment"
REST_FILE   = DATA_DIR / "restaurant_train.jsonl"
LAP_FILE    = DATA_DIR / "laptop_train.jsonl"
TEST_FILE   = DATA_DIR / "test.jsonl"

MODEL_DIR   = BASE_DIR / "aste_pipeline_model_lstm"
OUTPUT_FILE = BASE_DIR / "predictions_v7_lstm.jsonl"
...
LSTM_LR      = 5e-5     # LSTM trained from scratch
...
print(f"Model:  {MODEL_NAME} + BiLSTM")
...
if FAST_TEST:
    ...
    print("  [FAST_TEST] Truncated to 100 train / 20 test")

if len(train_data) == 0:
    raise FileNotFoundError("No training data found! Please make sure nlp_assignment folder has the jsonl datasets.")
...
# ════════════════════════════════════════════════════════════════════════════
# SECTION 3 — MODEL WITH LSTM
# ════════════════════════════════════════════════════════════════════════════

class ASTEModelLSTM(nn.Module):
    def __init__(self):
        super().__init__()
        ...
        # LSTM parameters
        lstm_hidden = hidden_size // 2  # Bidirectional will make output = hidden_size
        
        # BiLSTM Layer added on top of Transformer
        self.lstm = nn.LSTM(
    ...
    def forward(self, input_ids, attention_mask):
        ...
        transformer_hidden = out.last_hidden_state   # (B, L, H)
        
        # 2. LSTM Encoder
        # We pack/unpack sequences for better LSTM performance, or simply pass the tensor
        # since we have attention_mask and fixed MAX_LEN.
        lstm_out, _ = self.lstm(transformer_hidden) # (B, L, 2 * lstm_hidden)
        
        # 3. Task Heads
        hidden  = self.dropout(lstm_out)
        ...
        for b_idx in range(len(ids)):
            ...
            for (as_, ae) in asp_spans:
                ...
                for (os_, oe) in opn_spans:
                    dist = min(abs(as_ - oe), abs(os_ - ae))
                    if dist < best_dist:
                        best_dist = dist
                        best_opn  = (os_, oe)
...
if __name__ == "__main__":
    ...
    # LSTM parameters
    lstm_params = [p for n, p in model.lstm.named_parameters()]
    ...
    params = [
        ...
        {"params": lstm_params,             "lr": LSTM_LR,    "weight_decay": 0.01},
    ...
    print(f"  Encoder LR: {ENCODER_LR}  LSTM LR: {LSTM_LR}  Head LR: {HEAD_LR}")
