...
def patch_notebook(path):
    ...
    for cell in nb['cells']:
        if cell['cell_type'] == 'code':
            ...
            # 1. Fix ASTEDataset __getitem__ to not return padded tensors
        ...
        return {
            ...
            if "predict_with_generate=True," in src:
                src = src.replace("predict_with_generate=True,", "predict_with_generate=False, # disabled to prevent NaN val loss without compute_metrics func during Trainer loop")
            
            # 3. Reduce batch size to prevent OOM
            if "BATCH_SIZE       = 8" in src:
                src = src.replace("BATCH_SIZE       = 8", "BATCH_SIZE       = 4")
                src = src.replace("GRAD_ACCUM_STEPS = 4", "GRAD_ACCUM_STEPS = 8")
